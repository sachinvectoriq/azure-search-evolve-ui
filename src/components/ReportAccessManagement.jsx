///src/components/ReportAccessManagement.jsx
...
const ReportAccessManagement = ({ adminName = "Admin" }) => {
  ...
  const fetchAccessUsers = async () => {
    setIsLoading(true);
    try {
      const response = await fetch(`${API_BASE}/get_reports_access`);
      if (!response.ok) throw new Error('Failed to fetch access users');
  ...
  const handleAddUser = async () => {
    ...
    try {
      const response = await fetch(`${API_BASE}/add_reports_access_user`, {
        method: 'POST',
  ...
  const handleDeleteUser = async (id, userName) => {
    ...
    try {
      const response = await fetch(`${API_BASE}/delete_reports_access`, {
        method: 'DELETE',
        headers: { 'Content-Type': 'application/json' },
              ...
              <tbody className="bg-white">
                {accessUsers.map((user) => (
                  <tr key={user.id} className="border-b border-gray-200 hover:bg-gray-50 transition-colors">
                    <td className="py-4 px-5 text-gray-800 font-medium">{user.name}</td>
                    <td className="py-4 px-5 text-gray-600">{user.email}</td>
                    <td className="py-4 px-5 text-gray-600">{user.granted_by}</td>
                    <td className="py-4 px-5 text-gray-600 text-sm">{formatDate(user.permission_granted_at)}</td>
